...
static void gplotter_app_activate(GApplication *app) {
	GPlotterAppWindow *win;

	win = gplotter_app_window_new(GPLOTTER_APP(app));

	gtk_window_present(GTK_WINDOW(win));
}

static void gplotter_app_open(GApplication *app, GFile **files,
			      gint n_files, const gchar *hint) {
}

static gboolean gplotter_app_local_cmdline(GApplication *app, 
	gchar ***args, int *exit_status) {
...
GPlotterApp *gplotter_app_new(void) {
	return g_object_new(GPLOTTER_APP_TYPE,
